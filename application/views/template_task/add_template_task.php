...
<div class="coInputMainBlock">
	...
	<fieldset>
		...
		<?php
			...
			if ($task->isNew()) {
				render_member_selectors($projectTask->getObjectTypeId(), $genid, null, array('select_current_context' => true, 'listeners' => $listeners));
			} else {
				render_member_selectors($projectTask->getObjectTypeId(), $genid, $task->getMemberIds(), array('listeners' => $listeners));
			} 
		?>
	</fieldset>
	</div>
    	...
    	<div style="padding-top:4px">
    		...
    		og.addParentTask = function(before, obj) {
    			...
    			div.innerHTML =
    				...
    				'<a href="#" onclick="og.removeParentTask(this.parentNode)" class="removeDiv" style="display: block;">'+lang('remove')+'</div><div class="clear"></div>';
